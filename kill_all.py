...
class ComposeDown:
    ...
    def observability_compose_down(self):
        compose_down = subprocess.run(['docker', 'compose', 'down', '-v'], capture_output=True)

    def services_compose_down(self):
        compose_down = subprocess.run(['docker', 'compose', '-f', './generated/docker_compose.yml', 'down'], capture_output=True)
